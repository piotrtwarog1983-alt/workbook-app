...
export function VideoPlayer({ pageNumber, minimal = false }: VideoPlayerProps) {
  ...
  // Wersja minimalna - tylko video z podstawowymi kontrolkami
  if (minimal) {
    return (
      <div className="relative w-full max-w-sm aspect-[9/16] rounded-lg overflow-hidden bg-black">
        {isLoading && !error && (
        ...
        {error && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-white/50 text-sm text-center px-4">{error}</p>
          </div>
        )}
        ...
        {/* Overlay play button gdy paused */}
