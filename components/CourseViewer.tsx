...
export function CourseViewer({ courseSlug }: CourseViewerProps) {
  ...
  useEffect(() => {
    // Użyj mock data zamiast API - do pracy nad wyglądem (bez bazy danych)
    setTimeout(() => {
      setCourse(MOCK_COURSE)
      setLoading(false)
    }, 500) // Symulacja ładowania

    // Zakomentuj poniższe, jeśli chcesz używać tylko mock data:
    /*
  ...
  const isThreeImagesTopText = content?.type === 'three-images-top-text'
    ...
    },
    {
      ...
      content: JSON.stringify({
        ...
        imageUrl: '/course/strona 4/Foto/start.jpg', // Zdjęcie z zielonym deserem
        overlayText: 'Start',
        textPosition: 'bottom-center', // Tekst na dole, wyśrodkowany
      }),
      tips: JSON.stringify([]),
    },
    {
      ...
      content: JSON.stringify({
        ...
        imageUrl: '/course/strona 5/Foto/6.jpg', // Zdjęcie białego deseru
        textFile: '/api/course-content/5/PL', // Tekst z pliku
      }),
      tips: JSON.stringify([]),
    },
    {
    ...
    },
    {
      ...
      content: JSON.stringify({
        ...
        overlayText: 'Światło',
        textPosition: 'bottom', // 30% od dolnej krawędzi
      }),
      tips: JSON.stringify([]),
    },
    {
